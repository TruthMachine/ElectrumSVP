...
def is_tails() -> bool:
    """Detect if running on TAILS OS."""
    try:
        with open("/etc/os-version", "r") as f:
            content = f.read()
        return "TAILS" in content
    except Exception:
        return False


def set_tails_proxy(config: "SimpleConfig"):
    """Set Tor SOCKS5 proxy if running on TAILS."""
    import socket
    import time
    if is_tails():
        # wait up to 5 seconds for Tor to be ready
        for _ in range(5):
            try:
                ...
                time.sleep(1)
        config.set_key("proxy", "socks5:127.0.0.1:9050")
